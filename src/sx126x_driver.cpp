...
void sx126x_setSPI(SPIClass &SpiObject, uint32_t frequency)  // this is called only when we do not use the default spi
{
...
bool sx126x_busyCheck(uint32_t timeout)
{
    ...
    while (digitalRead(sx126x_busy) == HIGH) {
        if (millis() - tPlus >2) {   // call yield() at least once every x (2) msec
            yield();
            tPlus = millis();
        }    
        if (millis() - t > timeout) return true;
    }    
    return false;
}
